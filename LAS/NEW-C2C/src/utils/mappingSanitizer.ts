 ...
 */
export const finalizeMappingData = (parsed: any, poDb: any): MappingData => {
    // Audit POs to 12 as per user's earlier requirement for exhaustive mapping
    const validPOs = poDb.slice(0, 12).map((p: any) => ({
        id: p.id,
    ...
    return {
        ...
        courseOutcomes: (Array.isArray(parsed.courseOutcomes) ? parsed.courseOutcomes : []).map((co: any) => ({
            ...co,
            hours: Number(co.hours) || 0
        })),
        programOutcomes: validPOs,
        lectures: (Array.isArray(parsed.lectures) ? parsed.lectures : []).map((l: any, i: number) => ({
            ...l,
        ...
        practicals: (Array.isArray(parsed.practicals) ? parsed.practicals : []).map((p: any, i: number) => ({
            ...p,
        ...
        pblActivities: (Array.isArray(parsed.pblActivities) ? parsed.pblActivities : []).map((pbl: any, i: number) => ({
            ...pbl,
        ...
        coPoMapping: typeof parsed.coPoMapping === 'object' ? parsed.coPoMapping : {},
        justifications: typeof parsed.justifications === 'object' ? parsed.justifications : {}
    };
};