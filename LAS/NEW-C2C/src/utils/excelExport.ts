...
export interface MappingData {
  ...
  courseOutcomes: { id: string; description: string; rbtLevel: string; hours: number }[];
  programOutcomes: { id: string; description: string }[];
  lectures: { lectureNo: number | string; unit: string; topic: string; hours: number; co: string; pos: string[] }[];
  practicals: { practicalNo: number | string; name: string; hours: number; mappedCOs: string[]; mappedPOs: string[]; assessment?: string }[];
  pblActivities: { activityNo: number | string; name: string; hours: number; mappedCOs: string[]; mappedPOs: string[]; assessment?: string }[];
  coPoMapping: Record<string, string[]>;
  coPoStrengths?: Record<string, Record<string, number>>;
  justifications?: Record<string, string>;
}

export function generateExcelExport(data: MappingData, fileName: string = "CO-PO-Mapping.xlsx") {
  // Create workbook
  ...
  const coData = [
    ...
    ...data.courseOutcomes.map((co: any) => [
      co.id,
      co.description,
      co.rbtLevel,
      co.hours,
  ...
  const poData = [
    ["PO ID", "Description"],
    ...data.programOutcomes.map((po: any) => [po.id, po.description])
  ];
  ...
  const matrixData: (string | number)[][] = [
    ["", ...data.programOutcomes.map((po: any) => po.id)],
    ...data.courseOutcomes.map((co: any) => [
      co.id,
  ...
  const pblData = [
    ...
    ...data.pblActivities.map((pbl: any) => [
      pbl.activityNo,
      pbl.name,
      pbl.hours,
      pbl.assessment || "",
